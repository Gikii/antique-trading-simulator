...
namespace AntiqueTradingSimulator.Agents
{
    ...
    public class TraderHelper
    {
        ...
        public static bool SellListing(TraderInventory inventory, Market.Market market, string listingId, string traderName)
        {
            if (!HasMarket(market, traderName)) return false;

            bool success = inventory.Sell(market, listingId);
